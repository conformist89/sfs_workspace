...
class CorrectionSet(object):
    ...
    def add_correction(self, correction):
        if isinstance(correction, dict):
            ...
            raise TypeError("Correction must be a Correction object or a dictionary, not {}".format(type(correction)))

    def write_json(self, outputfile):
        # Create the JSON object
        cset = schema.CorrectionSet(schema_version=schema.VERSION, corrections=self.corrections)
...
class pt_eta_correction(Correction):
    ...
    def set_workspace(self, file=None):
        if file is None:
            file = "./htt_scalefactors_UL_" + str(self.era).replace("UL", "") + ".root"
        self.workspace = WorkspaceReader(file)
                        ...
                        "content": [
                            {
                                ...
                                "input": "type",
                                "content": [
                                    {"key": "mc", "value": self.workspace.get_sfs_2D(pt, eta, self.name, "mc")},
                                    {"key": "emb", "value": self.workspace.get_sfs_2D(pt, eta, self.name, "emb")},
                                ],
                            }
    ...
    def generate_scheme(self, workspace_file=None):
        self.parse_config()
        self.setup_scheme()
        self.set_workspace(workspace_file)
...
class emb_doublemuon_correction(Correction):
    ...
    def parse_config(self):
        ...
        basename = str(os.path.basename(self.configfile)).split("_")[1].replace(".yaml", "")
